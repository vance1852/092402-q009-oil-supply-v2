...
- 线路接收日历按 IANA 时区配置当地营业日、交接窗口（可跨午夜）和节假日例外，发运时冻结日历版本并据此计算可接收的预计到达时刻，日历修订不改写在途承诺但可预览影响；
- 终端收货区分部分到货、质量待验和最终签收，扫描级幂等防止重复累计，只有最终签收数量结转提名；
- 超时告警以落库的预计到达时刻为准，服务重启后继续有效；
...
健康检查为 `GET /health`。除健康检查外，请求通过 `X-Actor-Id` 携带操作者编号。可用接口覆盖报价、设施、线路、接收日历、停运事件、库存批次、提名、能力分配、发运、终端收货、超时转运、日历影响预览、供应情景和审计链。服务重启后，SQLite 中的业务状态和历史版本会继续保留。
